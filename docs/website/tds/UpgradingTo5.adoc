...
* *SectionFeature* and *SectionFeatureCollection* have been renamed to *TrajectoryProfileFeature*,
  *TrajectoryProfileFeatureCollection* for clarity.
...
* In all the Point Feature API classes, *DateUnit, Date*, and *DateRange* have been replaced by
    *CalendarDateUnit*, *CalendarDate*, and *CalendarDateRange*
* In `PointFeature` implementations and subclasses, all constructors now have a `DsgFeatureCollection` parameter.
  The `CalendarDateUnit` parameter has been removed, as it can be obtained from the `DsgFeatureCollection`.
* In `PointFeature`, removed deprecated method `getData()`. Use `getFeatureData()` or `getDataAll()` instead.
* In `PointFeature`, removed methods `getObservationTimeAsDate()` and `getNominalTimeAsDate()`.
  Use `getObservationTimeAsCalendarDate().toDate()` and `getNominalTimeAsCalendarDate().toDate()` instead.
